...
public final class LlamaRuntime {
    ...
    static void check_tensors(MemorySegment segment, boolean checkTensors) {
        switch (runtime) {
            case MACOSX_AARCH_64 -> io.gravitee.llama.cpp.macosx.aarch64.llama_model_params.check_tensors$set(segment, checkTensors);
    ...
    static MemorySegment llama_model_load_from_file(MemorySegment modelPath, MemorySegment modelParams) {
        return switch (runtime) {
            case MACOSX_AARCH_64 -> io.gravitee.llama.cpp.macosx.aarch64.llama_h.llama_model_load_from_file(modelPath, modelParams);
    ...
    static int llama_token_to_piece(MemorySegment vocab, int token, MemorySegment buf, int length, int lstrip, boolean special) {
        return switch (runtime) {
            case MACOSX_AARCH_64 -> io.gravitee.llama.cpp.macosx.aarch64.llama_h.llama_token_to_piece(vocab, token, buf, length, lstrip, special);
            case LINUX_X86_64 -> io.gravitee.llama.cpp.linux.x86_64.llama_h.llama_token_to_piece(vocab, token, buf, length, lstrip, special);
            default -> throw new IllegalStateException("Unexpected value: " + runtime);
        };
    }
    ...
    static MemorySegment llama_sampler_init_penalties(int penaltyLastN, float penaltyRepeat, float penaltyFreq, float penaltyPresent) {
        return switch (runtime) {
            case MACOSX_AARCH_64 -> io.gravitee.llama.cpp.macosx.aarch64.llama_h.llama_sampler_init_penalties(penaltyLastN, penaltyRepeat, penaltyFreq, penaltyPresent);
            case LINUX_X86_64 -> io.gravitee.llama.cpp.linux.x86_64.llama_h.llama_sampler_init_penalties(penaltyLastN, penaltyRepeat, penaltyFreq, penaltyPresent);
            default -> throw new IllegalStateException("Unexpected value: " + runtime);
        };
    }
    ...
    static void n_threads_batch(MemorySegment segment, int nThreadBatch) {
        switch (runtime) {
            case MACOSX_AARCH_64 -> io.gravitee.llama.cpp.macosx.aarch64.llama_context_params.n_threads_batch$set(segment, nThreadBatch);
            case LINUX_X86_64 -> io.gravitee.llama.cpp.linux.x86_64.llama_context_params.n_threads_batch$set(segment, nThreadBatch);
            default -> throw new IllegalStateException("Unexpected value: " + runtime);
        }
    }
    ...
    static int llama_chat_apply_template(MemorySegment tmpl, MemorySegment chat, long n_msg, boolean add_ass, MemorySegment buf, int length) {
        return switch (runtime) {
            case MACOSX_AARCH_64 -> io.gravitee.llama.cpp.macosx.aarch64.llama_h.llama_chat_apply_template(tmpl, chat, n_msg, add_ass, buf, length);
            case LINUX_X86_64 -> io.gravitee.llama.cpp.linux.x86_64.llama_h.llama_chat_apply_template(tmpl, chat, n_msg, add_ass, buf, length);
            default -> throw new IllegalStateException("Unexpected value: " + runtime);
        };
    }
    ...
    /* Tokenizer */
    static int llama_tokenize(MemorySegment vocab, MemorySegment text, int text_len, MemorySegment tokens, int n_tokens_max, boolean add_special, boolean parse_special){
        return switch (runtime) {
            case MACOSX_AARCH_64 -> io.gravitee.llama.cpp.macosx.aarch64.llama_h.llama_tokenize(vocab, text, text_len, tokens, n_tokens_max, add_special, parse_special);
            case LINUX_X86_64 -> io.gravitee.llama.cpp.linux.x86_64.llama_h.llama_tokenize(vocab, text, text_len, tokens, n_tokens_max, add_special, parse_special);
            default -> throw new IllegalStateException("Unexpected value: " + runtime);
        };
    }
