// Generated by jextract

package io.gravitee.llama.cpp.macosx.aarch64;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet lang=c :
 * struct ggml_backend_feature {
 *     char* name;
 *     char* value;
 * };
 * }
 */
public class ggml_backend_feature {

    public static MemoryLayout $LAYOUT() {
        return constants$119.const$5;
    }
    public static VarHandle name$VH() {
        return constants$120.const$0;
    }
    /**
     * Getter for field:
     * {@snippet lang=c :
     * char* name;
     * }
     */
    public static MemorySegment name$get(MemorySegment seg) {
        return (MemorySegment)constants$120.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet lang=c :
     * char* name;
     * }
     */
    public static void name$set(MemorySegment seg, MemorySegment x) {
        constants$120.const$0.set(seg, x);
    }
    public static MemorySegment name$get(MemorySegment seg, long index) {
        return (MemorySegment)constants$120.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void name$set(MemorySegment seg, long index, MemorySegment x) {
        constants$120.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle value$VH() {
        return constants$120.const$1;
    }
    /**
     * Getter for field:
     * {@snippet lang=c :
     * char* value;
     * }
     */
    public static MemorySegment value$get(MemorySegment seg) {
        return (MemorySegment)constants$120.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet lang=c :
     * char* value;
     * }
     */
    public static void value$set(MemorySegment seg, MemorySegment x) {
        constants$120.const$1.set(seg, x);
    }
    public static MemorySegment value$get(MemorySegment seg, long index) {
        return (MemorySegment)constants$120.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void value$set(MemorySegment seg, long index, MemorySegment x) {
        constants$120.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


