...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    // With prebuilt binaries, backends (CPU, Metal, RPC, etc.) are registered
    // statically when the shared libraries are loaded by LlamaLibLoader.
    // ggml_backend_load_all_from_path attempts dynamic loading on top of that.
    // When using RPC, we skip it since only the RPC backend is needed and
    // it's already registered.
    String rpcEndpoints = params.get("rpc");
    boolean useRpc = rpcEndpoints != null && !rpcEndpoints.isBlank();

    if (useRpc) {
      String[] endpoints = rpcEndpoints.split(",");
      for (String endpoint : endpoints) {
        String trimmed = endpoint.trim();
        if (!trimmed.isEmpty()) {
          System.out.println("Registering RPC server: " + trimmed);
          BackendRegistry.addRpcServer(ARENA, trimmed);
        }
      }
    } else {
    ...
    // When using RPC, restrict offloading to only RPC devices
    // so weights are not loaded on local GPU (e.g., Metal)
    if (useRpc) {
      var rpcDevices = BackendRegistry.getRpcDeviceHandles();
      if (rpcDevices.isEmpty()) {
        System.err.println(
          "Error: no RPC devices found after registering servers."
        );
        System.exit(1);
      }
      modelParameters.devices(ARENA, rpcDevices);
    }
    ...
    System.err.println(
      ...
      Model parameters:
        ...
        --rpc <endpoints>           Comma-separated RPC server endpoints for distributed inference
                                    Format: host1:port,host2:port (default: none)
                                    When set, model weights and KV-cache are offloaded exclusively
                                    to the remote RPC servers (local GPU is not used).
